...
class Featured extends React.Component {
  state={
    ...
    render() {
      ...
      let images = this.state.renderItems.map(item => {
      console.log("Item", item.name);
        return (
        
          <View style={{padding:10, marginTop:10}}>
            <TouchableOpacity>
              <Image style={{width: 250, height: 170, borderRadius:10}} source={{uri: item.uri}} />
            </TouchableOpacity>
          </View>
      ...
      let images1 = this.state.renderItems.map(item => {
        console.log("Item", item.name);
          return (
          
            <View style={{padding:10}}>
              <TouchableOpacity>
                <Image style={{width: 180, height: 100, borderRadius:10}} source={{uri: item.uri}} />
              </TouchableOpacity>
        ...
        let images2 = this.state.renderItems.map(item => {
          console.log("Item", item.name);
            return (
            
              <View style={{padding:10}}>
                <TouchableOpacity>
                  <Image style={{width: 120, height: 150, borderRadius:10}} source={{uri: item.uri}} />
                </TouchableOpacity>
                <Text style={{color:'white'}}>{item.name}</Text>
              </View>
          ...
          let images3 = this.state.renderItems.map(item => {
            console.log("Item", item.name);
              return (
            ...
            let images4 = this.state.renderItems.map(item => {
              console.log("Item", item.name);
                return (
